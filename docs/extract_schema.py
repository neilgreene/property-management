...
WHERE c.table_schema IN ('core','ghl','feed','gov','intake')
  AND cl.relkind = 'r'
...
WHERE n.nspname IN ('core','ghl','feed','gov','intake')
GROUP BY 1,2,3;
"""
...
WHERE n.nspname IN ('core','ghl','feed','gov','intake') AND cl.relkind = 'r'
ORDER BY 1,2;
"""
...
JOIN unnest(con.conkey) AS k(attnum) ON true
JOIN pg_attribute a  ON a.attrelid = cl.oid AND a.attnum = k.attnum
WHERE con.contype = 'f' AND n.nspname IN ('core','ghl','feed','gov','intake');
"""
